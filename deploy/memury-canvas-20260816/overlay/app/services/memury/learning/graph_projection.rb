  ...
  module Learning
    ...
    class GraphProjection
      ...
      STEP_KINDS = {
        ...
        "tutor_response" => ["tutor_response", "Q Graph"],
      ...
      def call
        ...
        current = node_ids.include?(stored_current) ? stored_current : projected_nodes.last.fetch("id")
        {
          ...
          "conversation_summary" => session.summary.to_s.presence,
          "conversation_key_points" => Array(session.metadata.to_h["q_graph_key_points"]).first(5),
          "conversation_turns" => steps.count { |step| step.kind == "student_question" },
          "writable" => true
        }.compact
      end
      ...
      def empty_graph
        {
          ...
          "writable" => assignment_ref.present?
        }
      end
      ...
      def step_title(step, default_title)
        ...
        return "Q Graph" if step.kind == "tutor_response"
      ...
      def step_summary(step, evidences)
        ...
        when "tutor_response"
          output = step.output.to_h.deep_stringify_keys
          safe_text([output["answer"], output["follow_up"]].compact_blank.join(" "), 920)
        when "validation"
      ...
      def relationship_for(step)
        return "从历史节点继续的学生分支" if step.kind == "student_question"
        return "Q Graph 对当前问题的文档内回应" if step.kind == "tutor_response"

        "当前 Session 的第 #{step.sequence} 步"
      end
